...
logistic_regression <- function(predictors, response, max_iterations)
{
  ones <- matrix(1,nrow(predictors),1)
  X <- as.matrix(cbind(ones, predictors))
  Y <- as.matrix(response)
  Beta <- ((inv(t(X) %*% X) %*% t(X)) %*% Y)
  for (itteration in 1:max_iterations)
  {
    P <- exp(X %*% Beta)/(1 + exp(X %*% Beta))
    one_minus_P <- 1-P
    W <- Diagonal(nrow(X), (P*one_minus_P))
    #Gradient <- as.matrix(unname(t(X) %*% (Y - P)))
    #Hessian <- as.matrix(unname(-(t(X) %*% W %*% X)))
    Beta <- Beta + (ginv(as.matrix(unname((t(X) %*% W %*% X)))) %*% as.matrix(unname(t(X) %*% (Y - P))))
  }
  return(t(Beta))
}

linear_model_betas <- matrix(0, nrow(laplacian_matrix), 11)
for(user in 1:104)
{
  ...
  linear_model_betas[user,] <- logistic_regression(predictors, response, 100)
}
```
